...
function mod:OnInitialize()
	...
	local editBox = CreateFrame("EditBox", "ChatterCopyEditBox", frame)
...
local function fixLine(line)
	line = gsub(line, "(|HBNplayer:%S-|k:)(%d-)(:%S-|h)(%S-)(|?h?)(:)", fixName)
	line = gsub(line, "(|TInterface(.*)|t)", "")
	return line
end

function mod:GetLines(frame)
	local regions = {frame.FontStringContainer:GetRegions()}
	local fontStrings = {}
	...
	sort(fontStrings, function(a, b) return a:GetBottom() > b:GetBottom() end)

	wipe(lines)
	for _, fontString in ipairs(fontStrings) do
		tinsert(lines, fixLine(tostring(fontString:GetText())))
	end
end
